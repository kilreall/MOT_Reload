...
def one_atom_sim(stv):
    ...
    while i < int(TSIM/dt):
        ...
        # fall new realization
        nt = np.arange(0, int(2*T/dt)+1)*dt # not sure about +1

        vz[i+1:i+int(2*T/dt)+1] = (vz[i]-g*nt)[1:]
        z[i+1:i+int(2*T/dt)+1] = (z[i]-g*nt**2/2+vz[i]*nt)[1:]

        vx[i+1:i+int(2*T/dt)+1] = (vx[i]+0*nt)[1:]
        x[i+1:i+int(2*T/dt)+1] = (x[i]+vx[i]*nt)[1:]

        vy[i+1:i+int(2*T/dt)+1] = (vy[i]+0*nt)[1:]
        y[i+1:i+int(2*T/dt)+1] = (y[i]+vy[i]*nt)[1:]
